...
def eval_preds(y_true_list, y_pred_list, PATH_RESULTS, idx2class=None):
    ...
    '''
    cf_mat = confusion_matrix(y_true_list, y_pred_list)
    ...
    chi_square = len(y_pred_list) * mcc ** 2
    print('Chi-Square: %f' % chi_square)

    proba = 0.95
    alpha = 1.0 - proba
    oddsratio, pvalue = fisher_exact(cf_mat)
    print('p-value: %f' % pvalue)

    results = {'tp': float(tp), 'tn': float(tn), 'fn': float(fn), 'fp': float(fp),
               'accuracy': accuracy, 'precision': precision, 'recal': recall,
               'f1-score': f1, 'specificity': specificity, 'mcc': mcc
               }
